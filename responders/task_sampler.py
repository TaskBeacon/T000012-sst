...
class TaskSamplerResponder:
    """Task-local sampler responder for SST simulation."""
    ...
    def act(self, obs: Observation) -> Action:
        ...
        if phase in {"instruction_text", "block", "goodbye", "block_feedback"}:
            ...
            if key is None:
                return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "reason": "no_valid_key"})
            return Action(key=key, rt_s=self.continue_rt_s, meta={"source": "sst_sampler", "phase": phase})

        if condition.startswith("go_"):
            if phase not in {"go_response_window", "stimulus", "response"}:
                return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "phase": phase, "outcome": "withhold"})
            if float(self._rng.random()) > self.p_hit_go:
                return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "go_miss"})
            ...
            if deadline is not None and rt_s > deadline:
                return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "late"})

            return Action(key=key, rt_s=rt_s, meta={"source": "sst_sampler", "condition": condition, "outcome": "go_hit"})
            ...
            if phase == "pre_stop_go_window":
                if float(self._rng.random()) < self.p_fail_stop:
                    ...
                    if key is not None:
                        ...
                        if deadline is not None and rt_s > deadline:
                            return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "late"})
                        return Action(key=key, rt_s=rt_s, meta={"source": "sst_sampler", "condition": condition, "outcome": "stop_fail_pre"})
                return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "hold_pre"})

            if phase == "stop_signal_window":
                if trial_id and trial_id in self._failed_stop_trial_ids:
                    return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "already_failed"})
                return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "stop_success"})

            return Action(key=None, rt_s=None, meta={"source": "sst_sampler", "condition": condition, "outcome": "withhold"})
